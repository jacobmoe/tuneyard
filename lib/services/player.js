...
function initialize(io) {
  events.on('newCurrentTrack', function (data) {
    ...
    // check if anyone is in the room before sending a new-track message
    if (io.sockets.adapter.rooms[data.playlistName]) {
      var messageParams = {playlistName: data.playlistName}
      messageParams.content = "Now playing: " + data.track.title

      Message.create(messageParams, function (err, message) {
        if (err) return
        io.to(data.playlistName).emit('messages:display', message)
      })
    }
  })
